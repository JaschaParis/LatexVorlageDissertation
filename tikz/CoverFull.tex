...
\node [anchor=center, text=RWTHblau, text width=150mm, rotate=90, align=center,font=\Large] (rueckentext) at ($(nullpunkt_ruecken)!0.5!(maxpunkt_ruecken)$) {\textsf{\titel}};

\node [anchor=west, text=RWTHblau, rotate=90, align=center,font=\Large] (rueckenautor) at ($(nullpunkt_ruecken) + (\ruecken/2,10)$) {\textsf{\autor}};

\input{content/cover_back_content}

\begin{scope}[on background layer]
	\draw [draw=none,fill=RWTHblau] (Institut.south west) rectangle (maxpunkt_aufschlag -| nullpunkt_ruecken);
	\draw [draw=none,fill=RWTHblau] (Name.south west) rectangle (Name.north east -| maxpunkt_aufschlag);
\end{scope}
% % % % % % % % % % % % % % % % % % % % % % % % %
\end{tikzpicture}
\ifdefined\INCLUDETIKZ
